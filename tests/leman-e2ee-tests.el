...
(ert-deftest leman--sync-callback-ignores-disconnected-session ()
  ;; A response may be queued while `leman-disconnect' deletes its
  ;; process.  It must not use the stopped agent or restart syncing.
  (let* ((session (make-leman-session :user (make-leman-user :id "@me:x.org")))
         (e2ee-calls 0) (syncs 0))
    (cl-letf (((symbol-function #'leman-e2ee--sync-changes)
               (lambda (&rest _) (cl-incf e2ee-calls)))
              ((symbol-function #'leman--sync)
               (lambda (&rest _) (cl-incf syncs)))
              (leman-auto-sync t)
              (leman-syncs nil)
